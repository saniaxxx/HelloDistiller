...
#else
	#define PRINT_TEMPERATURES() { sprintf_P(lcd_buffer, PSTR("C2 T=%i.%i,%i.%i,%i.%i"), DS_TEMP(TEMP_KUB)/10, DS_TEMP(TEMP_KUB)%10, DS_TEMP(TEMP_RK20)/10, DS_TEMP(TEMP_RK20)%10, DS_TEMP(TEMP_TSA)/10, DS_TEMP(TEMP_TSA)%10);\
								 my_lcdprint(lcd_buffer);\
	}
#endif
	...
	if (DispPage == 0)
	{
      ...
      break; 
		case 2: //Разгон
			...
			my_lcdprint(lcd_buffer); 
			if (tEndRectRazgon > 0) {sprintf_P(lcd_buffer, PSTR("\272\171\262a = %i.%i/%i.%i"), DS_TEMP(TEMP_KUB)/10, DS_TEMP(TEMP_KUB)%10, tEndRectRazgon/10, tEndRectRazgon%10); 
...
//		else  sprintf_P(lcd_buffer, PSTR("%02u:%02u:%02u OT\240OP \241O\247OB"), hour, minute, second);                   //00:00:00 ОТБОР ГОЛОВ 
      ...
      break;      	  
		case 2: //Ожидание закипания (прогреется дефлегматор)
			if (IspReg != 105)  sprintf_P(lcd_buffer, PSTR("%02u:%02u:%02u %1u-O PA\244\241OH"), hour, minute, second, V1); //00:00:00 1-О РАЗГОН
			else sprintf_P(lcd_buffer, PSTR("%02u:%02u:%02u OT\240OP \241O\247OB"), hour, minute, second);                  //00:00:00 ОТБОР ГОЛОВ
			my_lcdprint(lcd_buffer); 
			formTRZGDistill();
			break;
	...
	if (DispPage == 0)
	{
      ...
      break;
        ...
        sprintf_P(lcd_buffer, PSTR("Te\274\276.K\171\262a= %i.%i/%i.%i"), DS_TEMP(TEMP_KUB)/10, DS_TEMP(TEMP_KUB)%10, V2/10, V2%10);         
			...
			else { sprintf_P(lcd_buffer, PSTR("B\303x.\274o\346\275oc\277\304 =%4iW"), UstPower);            //Вых.мощность =  
        my_lcdprint(lcd_buffer);
        lcd.setCursor(0,3);   
...
void DisplayRazvar()
{
	if(DispPage == 0)
	  {     
		 if (IspReg == 114)  sprintf_P(lcd_buffer, PSTR("%02u:%02u:%02u OX\247.\244ATOPA"), hour, minute, second);       //02u:%02u:%02 ОХЛ.ЗАТОРА                                                           
     else 
...
void DisplayTestKLP() // *************** Тест клапанов *****************
{
    ...
    my_lcdprint(lcd_buffer); 
    lcd.setCursor(0,1);
    if(StateMachine == 2)  strcpy_P(str_off, PSTR("B\272\273.")); 
    else  strcpy_P(str_off, PSTR("O\277\272\273"));
...
void DisplayData() //************ РЕЖИМ ПРОСМОТРА **********************
{
	...
	case 0:
          ...
          sprintf_P(lcd_buffer, PSTR("Po\267\273\270\263 Bo\343\303 =   %4u"), U_VODA);                      //Розлив Воды =           
			...
			case 102: //****************** Термостат ************************         
				if (DispPage == 0)
				{
					...
					my_lcdprint(lcd_buffer);
          lcd.setCursor(0,3);
          if(StateMachine == 2)  strcpy_P(str_off, PSTR("HA\241PEB")); 
          else  strcpy_P(str_off, PSTR("      "));
        ...
        sprintf_P(lcd_buffer, PSTR("\251po\263e\275\304 H\250\241= %i"), U_NPG);             //Уровень НПГ =
			...
			//
			if (StateNPG == 1 && DispPage == 0) sprintf_P(lcd_buffer, PSTR("Ha\276o\273\275e\275\270e H\250\241...")); //Наполнение НПГ...
...
//		break;
	...
	case 312:
      ...
      sprintf_P(lcd_buffer, PSTR("\343\270c\277\270\273\273\307\345\270\270 =%3i"), (int)TekFraction + 1);  
        ...
        sprintf_P(lcd_buffer, PSTR("%1i-\271 \344pa\272\345\270\270 =%5im"), (int)nPopr + 1, (int)-TempFractionDist[nPopr]);   
			...
			if (flPopr == 1)  sprintf_P(lcd_buffer, PSTR("\251\264o\273=%3i*, %4iW"), (int)AngleFractionDist[nPopr], PowerFractionDist[nPopr]);  
			else  sprintf_P(lcd_buffer, PSTR("\251\264o\273=%3i , %4iW"), (int)AngleFractionDist[nPopr], PowerFractionDist[nPopr]);              
                                                                                                   //угол=%3i ,%4iW
			if (flPopr == 2)  sprintf_P(lcd_buffer, PSTR("%s%c"), lcd_buffer, '*');                      //угол=%3i ,%4iW
		}
		lcd.setCursor(0, 2);
		my_lcdprint(lcd_buffer); 
		break;
    ...
    my_lcdprint(lcd_buffer);                                  
		lcd.setCursor(0, 3);
		if (nPopr == 0) sprintf_P(lcd_buffer, PSTR("\252a\267a1"));                                    //фаза 1
		if (nPopr == 1) sprintf_P(lcd_buffer, PSTR("        \252a\267a2"));                            //фаза 2
		if (nPopr == 2) sprintf_P(lcd_buffer, PSTR("               \252a\267a3"));                     //фаза 3
		my_lcdprint(lcd_buffer);  
    lcd.setCursor(0, 2);
