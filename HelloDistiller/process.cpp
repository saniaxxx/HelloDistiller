...
void ProcessBeerCloneBrau()
{
	...
	case 2: // Температура Больше необходиомй
		...
		if (DS_TEMP(TEMP_TERMOSTAT) < (int)tempP[KlTek] * 10 && tempP[KlTek] <= 100) break;
	...
	case 3:
		if (tempP[KlTek] > 100)
		{
			...
			if (DS_TEMP(TEMP_DEFL) < TempDeflBegDistil && ds1820_devices>1 || (TempDeflBegDistil < 0 && DS_TEMP(TEMP_DEFL) < -TempDeflBegDistil) && tempP[KlTek] >= 110)  time2 = (int)timeP[KlTek] * 60;
...
void ProcessDistilDefl()
{
	...
	if ((flAlarmUroven || DS_TEMP(TEMP_KUB) >= tEndDistDefl || deltaPower >= Power) && StateMachine < 7)
	...
	case 2: // Ожидание, пока прогреется термометр в дефлегматоре
		...
		if (DS_TEMP(TEMP_DEFL) < TempDeflBegDistil && ds1820_devices>1 || (TempDeflBegDistil < 0 && DS_TEMP(TEMP_KUB) < -TempDeflBegDistil)) break;
	...
	case 5:// 50% воды из холодильника идет в дефлегматор.
	  // Если более 10 минут не вышли за пределы температуры, тогда увеличиваем мощность.
		...
		if (DS_TEMP(TEMP_DEFL) > TempDefl + DeltaDefl)
		{
			...
			if (DS_TEMP(TEMP_DEFL) < TempDefl - DeltaDefl) // Если в результате работы с 50% дефлегматором, температура понизилась переходим в режим работы без дефлегматора
			{
...
void ProcessDistillFractional()
{
	...
	{ // Если сработал датчик уровня, то переходим на следующую фракцию.
		TekFraction++;
		SecTempPrev = Seconds;
		flAlarmUroven = 0;
		CountAlarmUroven = 0;
		if (TekFraction < CountFractionDist) SetAngle(AngleFractionDist[TekFraction]);
		if (BeepStateProcess) my_beep(BEEP_LONG);
	}
	...
	case 3:// Ждем, пока температура в кубе не превысит установленную
		...
		if (TekFraction < CountFractionDist)
		{
			...
			{   // Для последующих фракций (считаем их телом) клапан тела открываем, клапан голов закрываем.
				KlOpen[KLP_SR] = 200;
				KlClose[KLP_SR] = 0;
				KlOpen[KLP_GLV_HVS] = 0;
...
void ProcessNBK()
{
	...
	case 4:
		...
		if (time2 == 0)
		{
			...
			if (MaxPressByPeriod > (int)minPressNBK * 5 + (int)deltaPressNBK && minPressNBK > 0)
			{ // Если превышено давление, то переходим в режим прекращения подачи браги.
			  // Если минимальное давление не установлено, то ситуацию игнорируем.
				StateMachine = 6;
				time2 = ((int)timePressNBK * 5) / 8;
				if (BeepStateProcess) my_beep(BEEP_LONG / 2);
				break;
			}
		...
		if (SpeedNBKDst > 125) SpeedNBKDst = 125;
	...
	case 6: // Превышение давления, ожидаем снижения давления, подача браги при этом прекращается.
		...
		if (time2 == 0)
		{
			...
			{
				if (deltaPressNBK > 0) SpeedNBKDst -= 1;
			...
			if (U_MPX5010 <= (int)minPressNBK * 5 || minPressNBK <= 0) // Если текущее давление стало меньше или равно минимальному, тогда заканчиваем прекращать подачу браги 
...
void ProcessNDRF()
{
	...
	case 4: // Отбор голов
		...
		{
			if (DS_TEMP(TEMP_KUB) < tEndRectOtbGlv)
	...
	case 6: // Отбор СР
	  ...
	  // Устанавливаем ШИМ отбора СР
		...
		if (ProcChimSR > 0) KlOpen[KLP_SR] = ((timeChimRectOtbSR / 10)*ProcChimSR) / 10;
		else KlOpen[KLP_SR] = 0;
...
void ProcessRectif()
{
	...
	{ // Переводим автомат в стадию завершения 
		...
		else
		{
			...
			flAlarmUroven = 0;
			CountAlarmUroven = 0;
			if (TekFraction < CountFractionRect) SetAngle(AngleFractionRect[TekFraction]);
		}
	}
	...
	case 6: // Отбор СР
	  ...
	  // Работа Клапана отбора голов зависит от работы царги пастеризации
		...
		if (DS_TEMP(TEMP_RK20) >= tStabSR + tDeltaRect) // Если текущая температура превысила базовую 
		{
			...
			if (TimeRestabKolonna < 0) time1 = -TimeRestabKolonna;
			else time1 = TimeRestabKolonna;
		...
		else // if (DS_TEMP(TEMP_RK20)>=tStabSR+tDeltaRect)
		{
			...
			if (Seconds - SecTempPrev > TimeAutoIncCHIM)
			{
				...
				if (ProcChimSR > minProcChimOtbSR)
				{
					...
					if (ProcChimSR > 100) ProcChimSR = 100;
				}
				SecTempPrev = Seconds;// Запомним время, когда стабилизировалась температура
			}
