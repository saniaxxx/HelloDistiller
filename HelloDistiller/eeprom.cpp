...
{ 
  ...
  eeWriteChar(ip[3]);
  eeWriteInt(ipPort);
  for(i=0;i<10;i++) eeWriteChar(idDevice[i]);
  for(i=0;i<12;i++) eeWriteChar(my_phone[i]);
  ...
  for(i=0;i<11;i++) eeWriteChar(WiFiAP[i]);
  for(i=0;i<11;i++) eeWriteChar(WiFiPass[i]);
...
void readEEPROM()
{
  ...
  if (TempZSPSld>100 || TempZSPSld<0) TempZSPSld=68;
  TempZSP=eeReadChar();
  if (TempZSP>100 || TempZSP<0) TempZSP=68;
  PowerNBK=eeReadInt();
  ...
  if (IncrementCHIM==-1) IncrementCHIM=0;
  ...
  if (TimeRestabKolonna==-1) TimeRestabKolonna=3600;
  ...
  {
    ctmp=eeReadChar();
    if (ctmp!=255)  my_phone[i]=ctmp;
  }

  my_phone[12]=0;
  AlarmMPX5010=eeReadInt();
  if (AlarmMPX5010==-1) AlarmMPX5010=0;

  FlAvtonom=eeReadChar();
  if (FlAvtonom==-1) FlAvtonom=0;
  timeOpenBRD=eeReadChar();
  if (timeOpenBRD==255) timeOpenBRD=2;
  ...
  {
    ctmp=eeReadChar();
    if (ctmp!=255)  WiFiAP[i]=ctmp;
  }
  WiFiAP[11]=0;
  for(i=0;i<11;i++) 
  {
    ctmp=eeReadChar();
    if (ctmp!=255)  WiFiPass[i]=ctmp;
  }
  WiFiPass[11]=0;
  ...
  for(i=0;i<MAX_CNT_FRACTION_DIST;i++) 
  {
    ...
    if (TempFractionDist[i]==-1)  TempFractionDist[i]=0;
    PowerFractionDist[i]=eeReadInt();
    if (PowerFractionDist[i]==-1)  PowerFractionDist[i]=0;
    AngleFractionDist[i]=eeReadChar();
    if (AngleFractionDist[i]==255)  AngleFractionDist[i]=0;
  }
  ...
  for(i=0;i<MAX_CNT_FRACTION_RECT;i++) 
  {
    ...
    if (TempFractionRect[i]==-1)  TempFractionRect[i]=0;
    AngleFractionRect[i]=eeReadChar();
    if (AngleFractionRect[i]==255)  AngleFractionRect[i]=0;
  }

  TempHLDZatorBrog1=eeReadChar();
  if (TempHLDZatorBrog1>100 || TempHLDZatorBrog1<0) TempHLDZatorBrog1=36;

  PowerPhase[0]=eeReadInt();
  if (PowerPhase[0]<0 || PowerPhase[0]>10000) PowerPhase[0]=Power;
  PowerPhase[1]=eeReadInt();
  if (PowerPhase[1]<0 || PowerPhase[1]>10000) PowerPhase[1]=0;
  PowerPhase[2]=eeReadInt();
  if (PowerPhase[2]<0 || PowerPhase[2]>10000) PowerPhase[2]=0;
  if ((PowerPhase[0]+PowerPhase[1]+PowerPhase[2])<=0) PowerPhase[0]=Power;

  KtPhase[0]=eeReadChar();
  if (KtPhase[0]>250) KtPhase[0]=100;
  KtPhase[1]=eeReadChar();
  if (KtPhase[1]>250) KtPhase[1]=100;
  KtPhase[2]=eeReadChar();
  if (KtPhase[2]>250) KtPhase[2]=100;
  ...
  if (minPressNBK==-1) minPressNBK=26;
  deltaPressNBK=eeReadChar();
  if (deltaPressNBK==-1) deltaPressNBK=25;
  timePressNBK=eeReadChar();
  if (timePressNBK==-1) timePressNBK=36;

  UprNasosNBK=eeReadChar();
  if (UprNasosNBK==-1) UprNasosNBK=1;
  ProcChimOtbCP=eeReadInt();
  if (ProcChimOtbCP==-1) ProcChimOtbCP=0;
  PIDTime==eeReadChar();
  if (PIDTime==255) PIDTime=15;
  ...
  if (timePressAtm==255) timePressAtm=25;
#endif
