...
char PIDTemp[3] = {
    20, 10, 30
}; // Пропорциональный коэффициент пид-регулятора температуры [0]
...
int FlState = 0; // Флаг состояния конечного автомата обработки нажатия кнопок клавиатуры
...
unsigned int timeChimRectOtbGlv = 42000; // Шим отбора голов
int ProcChimOtbGlv = 4; // Процент ШИМ отбора голов
...
char begProcChimOtbSR = 70; // Начальный ШИМ отбора СР
...
char tempP[MAX_CNT_PAUSE] = {
    10, 35, 45, 50, 58, 64, 70, 71, 110, 0, 0, 0, 0, 0, 0, 0
}; // Температурные паузы в градусах (первое значение - это время работы насоса).
unsigned char timeP[MAX_CNT_PAUSE] = {
    2, 15, 15, 20, 30, 60, 30, 0, 120, 0, 0, 0, 0, 0, 0, 0
}; // Время паузы в минутах (если 0, то это значит ожидаем действия пользователя (первое значение - это время паузы насоса).
