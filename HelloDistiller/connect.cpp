...
void SendWiFi8266()
{
    ...
    case 160:
        timeWaitGPRS = 10;
        if (FlToGSM == 10)
            sprintf_P(my_gprs_buffer, PSTR("AT+CIPSTART=\"UDP\",\"%u.%u.%u.%u\",%u"), (unsigned int)ip[0], (unsigned int)ip[1], (unsigned int)ip[2], (unsigned int)ip[3], ipPort);
        if (FlToGSM == 11)
            sprintf_P(my_gprs_buffer, PSTR("AT+CIPSTART=\"TCP\",\"%u.%u.%u.%u\",%u"), (unsigned int)ip[0], (unsigned int)ip[1], (unsigned int)ip[2], (unsigned int)ip[3], ipPort);
        // Подключение
        GSM_SERIAL.println(my_gprs_buffer);
        flGPRSState = 161;
    case 161:
...
void SendGPRS()
{
    ...
    case 10: // Пытаемся подключить интернет
        ...
        if (FlToGSM == 2)
            sprintf_P(my_gprs_buffer, PSTR("at+cgdcont=1,\"IP\",\"internet\""));
        else if (FlToGSM == 3)
            sprintf_P(my_gprs_buffer, PSTR("at+cgdcont=1,\"IP\",\"internet.beeline.ru\""));
        else if (FlToGSM == 4)
            sprintf_P(my_gprs_buffer, PSTR("at+cgdcont=1,\"IP\",\"internet.mts.ru\""));
        else if (FlToGSM == 5)
            sprintf_P(my_gprs_buffer, PSTR("at+cgdcont=1,\"IP\",\"internet.rt.ru\""));
        else if (FlToGSM == 6)
            sprintf_P(my_gprs_buffer, PSTR("at+cgdcont=1,\"IP\",\"internet.tele2.ru\""));
...
void AnalyseCommand()
{
...
#endif
            flGPRSState = 0;
        } else if (ErrGPRSConnectInernet > 20) {
#ifdef TESTGSM
...

    {
        ...
        else if (strstr(my_rx_buffer, ":Error")) {
            flGPRSState = 50;
...
#endif
    {
        ...
        if (strstr(my_rx_buffer, ":Error")) {
            ...
            if (flGPRSState == 21) {
                if (flNeedRefrServer) { // Если это разовая отправка
...
#endif
    {
        ...
        if (cmdGPRS > 'A') {
            ...
            if (cmdGPRS == 'T')
                StateMachine = 1; // Старт процесса
            if (cmdGPRS == 'P')
                StateMachine = 100; // Стоп процесса
            if (cmdGPRS == 'E')
                flNeedRefrServer = 1; // Если подана команда прекратить постоянный обмен,то включаем режим разовой отправки, после которой прекратится постоянный обмен.
            else
                flNeedRefrServer = 0; // Иначе отключаем режим разовой отправки.
            ScanKbd();
            timeGPRS = 120;
        }
    ...
    {
        flRing = 0;
        if (strstr(my_rx_buffer, my_phone))
            flRing = 1;
#ifdef TESTGSM
    ...
    if (flGPRSState == 63 && strlen(my_rx_buffer) > 40) {
        ...
        if (vid_sms) { // Если найден номер телефона, тогда выделяем информацию из него
...
void ProcessGSM()
{
    ...
    while (GSM_SERIAL.available() > 0) {
        ...
        if (s_rx == '>' && (flGPRSState == 19 || flGPRSState == 71 || flGPRSState == 163 || flGPRSState == 73)) {
            ...
            if (pos_rx_buffer >= MY_RX_BUFFER_SIZE)
                pos_rx_buffer = 0;
