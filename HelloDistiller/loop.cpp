...
void loop() 
{
  ...
  if (StateVolts!=1)                 
  {
    ...
    if (flNeedTemp==1)
    {
      ...
      if (StepOut==0)
      {
        ...
        flNeedTemp=0;
        for (j=0;j<ds1820_devices;j++) ds1820_flread[j]=0;
        StepOut=1;
      }
      else
      {
            ...
            if (ds1820_flread[j]==0)
            {
              ...
              if (ds18b20CRC!=data[8]) //  Ошибка чтения, записываем признак ошибки 
              {
                ...
                {             
                  ...
                  if (CntErrDs18>30) DispDopInfo=4;  // Устанавливаем признак вывода информации об ошибке если ошибок больше 4
        ...
        if (!flErrDs18) // Если не было ошибок, то сбрасываем счетчики.
        {
          ...
          if (DispDopInfo==4) DispDopInfo=0;
        }
        else
        {
  ...
  if (flNeedAnalyse==1)
  {
    ...
    case 103: //Текущее состояние - регулятор мощности
      ...
      if (flAutoDetectPowerTEN==1 && StateMachine<100) UstPower=9999;
      break;
    case 108:
    case 113:
      ProcessRazvarZerno();
  ...
  if (StateVolts==2)        
  {
    ...
    if (!flCorrASC712) // Расчет напряжения
    {
      ...
      if (FindKt>200 || It==-100) 
      {
        ...
        {   
          ...
          else DispDopInfo=6;              // и в том случае когда не установлено, что не нужно выводить предупреждение о низком уровне напряжения
#if NUM_PHASE>1
          if (FindKt>=200)
          {
      ...
      if (FindKtKLP<1000)
      {
        ...
        if (TimeOpenKLP<70) TimeOpenKLP=0; 
        TimeOpenKLP*=10;
      ...
      if (TimeOpenKLP<=10) TimeOpenKLP=9999; // Если не нашли нужную мощность, то устанавливаем максимальную.
      if (flAutoDetectPowerTEN) TimeOpenTriac=9999;
    }
    ...
    if (b_value[0]>b_size[0] && UstPower<Power && !flNoAlarmLowPower) DispDopInfo=3; // Выводим на экран информацию о низком напряжении если находимся не в режиме разгона.
    else
